...
public abstract class BluetoothConnection
{
    ...
    public void connect(String address, UUID uuid) throws IOException {
        ...
        if (socket == null) {
            Log.e("ADDED", "socket connection not established");
        ...
        try {
            socket.connect();
            Log.e("ADDED", "Connected Rfcomm");

        } catch (IOException e) {
            Log.e("ADDED", e.getMessage());
            try {
                Log.e("ADDED", "trying fallback...");
                socket =(BluetoothSocket) device.getClass().getMethod("createRfcommSocket", new Class[] {int.class}).invoke(device,1);

                socket.connect();
                Log.e("ADDED", "Connected serial UDID");

            } catch (Exception e2) {
                Log.e("ADDED", "Couldn't establish Bluetooth connection!");
                Log.e("ADDED", e.getMessage());
                Log.e("ADDED", "FinalCheck:"+socket.isConnected());
                throw new IOException("socket connection not established");
            }
        }
